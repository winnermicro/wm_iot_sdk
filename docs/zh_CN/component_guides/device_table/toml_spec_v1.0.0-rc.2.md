...
所有上面未列出的其它转义序列都是保留的，如果它们被使用了，TOML 将产生一个错误。  
...
当一行的最后一个非空白字符是反斜杠 `\` 时，它会连同它后面的所有空白（包括换行）一起被去除，直到下一个非空白字符或结束引号为止。  
...
因此，对于二进制数据，建议使用 Base64 或其它合适的 ASCII 或 UTF-8 编码。  
...
任意 64 位有符号整数（从 −2^63 到 2^63-1）应被接受并进行无损处理。如果不能无损表示整数，则必须报告错误。
...
要求毫秒精度。
小数秒的进一步精度取决于实现。  
如果该值包含的精度超出了实现所能支持的精度，则超出精度的部分必须被截断，而不能四舍五入。  
...
要求毫秒精度。
小数秒的进一步精度取决于实现。  
如果该值包含的精度超出了实现所能支持的精度，则超出精度的部分必须被截断，而不能四舍五入。小数秒的精度取决于实现，但至少应当能够精确到毫秒。  
