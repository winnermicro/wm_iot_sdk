...
* Whitespace refers to a tab (0x09) or a space (0x20).
* A Newline  is either LF (0x0A) or CRLF (0x0D 0x0A).
...
A bare key must be non-empty, but an empty quoted key is allowed (Although 
...
Whitespace around dot-separated parts is ignored, however, best practice is not to
...
Any Unicode character may be escaped using the `\uXXXX` or `\UXXXXXXXX` forms.
The escape codes must be valid Unicode [scalar
values](http://unicode.org/glossary/#unicode_scalar_value).

All other escape sequences not listed  that are not listed above are reserved; if they are used, TOML
should produce an error.

Sometimes you need to express passages of text (e.g. translation files) or you might
want to break up a very long string into multiple lines. TOML makes this easy.
