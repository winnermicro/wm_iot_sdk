...
``shell_api`` module. It allows for testing and exploring the Bluetooth Mesh APIs through an
...
The Bluetooth Mesh shell subsystem relieson the application to create the composition data and do
the mesh initialization.

Applications
**************

The Bluetooth Mesh shell subsystem is most easily used through the Bluetooth Mesh shell application  located
under ``tests/bluetooth/mesh_shell``.  For information on how to connect to and interact with the Bluetooth Mesh shell application, 
please refer to the :func:`shell_api` 

Basic Usage
***********

The Bluetooth Mesh shell subsystem adds a ``mesh`` command  with a set of subcommands.Ensure that ``mesh init`` 
is called each time the device is powered on before invoking any other Bluetooth mesh shell commands::

	uart:~$ mesh init

This ensures that all available logs are printed to the shell output.

Pre-configuration
======================

Mesh nodes must be configured to become part of a network. This is only necessary when the device is powered on for the first time, 
as the device will remember its configuration data between restarts.
...
the device into an existing network, enable unprovisioned beacons using
...
Once the mesh node is part of a network, its transmission parameters can be controlled by regular configuration commands:
...
``mesh prov local`` command above), we can perform self-configuration through any 
:ref:`bluetooth_mesh_shell_cfg_cli` command.

A good first step is to read the node's own composition data::

	uart:~$ mesh models cfg get-comp

This will print a list of the composition data of the node, including a list of its model IDs.
...
For example, to send a Generic OnOff Set message, dial::

	uart:~$ mesh test net-send 82020100

.. note::
	All multibyte fields in model messages, except for opcodes, are in little-endian format.

The message will be sent to the current destination address, using the current network and
application key indexes. As the destination address defaults  to the local unicast address by default,
...
When the destination address is set to ``0xffff``, any other mesh nodes in the network with the
configured network and application key will receive and process the messages we send.
...
Sending raw mesh packets is a good way to test the implementation of model message handlers, as it can be done without implementing sending models. 
By default, only the reception of model messages can be tested this way, as the Bluetooth Mesh shell only includes basic models. 
To receive packets in a mesh node, you must add a model with a valid list of opcode handlers to the composite data in``subsys/bluetooth/mesh/shell.c``, and print the
...
.. note::
	...
	the application. Not all features are enabled by default. By invoking ``mesh`` without any parameters, 
	you can display a list of available Bluetooth mesh shell commands within the shell.
...
-------------
	...
	Initializes the mesh shell. Which must be run before any other mesh commands.

``mesh reset-local``
--------------------

	Resets the local mesh node to its initial unset state. This command will also clear
...
-----------------------------
	...
	Gets or sets the message destination address. The destination address determines where mesh
...
-------------------------------
	...
	Gets or sets the message network index. which determines which network key is used to encrypt mesh packets sent with the shell,

    but it has no impact on modules outside shell control. The network key must already be added to the device through configuration or a configuration client.

	* ``NetKeyIdx``: If present, sets a new network index. If omitted, prints the current network index.


``mesh target app [AppKeyIdx]``
-------------------------------

	Gets or sets the message application index. The application index determines which application
	key is used to encrypt mesh packets that are sent with the shell, but it has no effect on
...
-----------------
	...
	Performs a poll to the friend node to receive any pending messages. Only available when LPN
	is enabled.
...
-----------------------------------
	...
	Sends a raw mesh message with the current destination address, network and application index.
...
-------------------------------------------
	...
	Sets the IV update test mode. In test mode, the IV update timing requirements are bypassed.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Registers a new Health Server Fault for the Linux Foundation Company ID.

	* ``FaultID``:  The fault ID to register (``0x0001`` to ``0xFFFF``)


``mesh test health-srv del-fault [FaultID]``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

	Deletes a registered Health Server faults for the Linux Foundation Company ID.
...
``CONFIG_BT_MESH_PROVISIONEE`` configuration option must be enabled, along with the
``CONFIG_BT_MESH_PB_GATT`` option.

``mesh prov pb-gatt <Val(off, on)>``
------------------------------------

	Starts or stop advertising a connectable unprovisioned beacon. The connectable unprovisioned
...
``CONFIG_BT_MESH_PROVISIONEE`` configuration option must be enabled, along with the
``CONFIG_BT_MESH_PB_ADV`` option.

``mesh prov pb-adv <Val(off, on)>``
-----------------------------------

	Starts or stop advertising the unprovisioned beacon. The unprovisioned beacon allows the mesh
...
To allow a device to provision devices, the ``CONFIG_BT_MESH_PROVISIONER`` and
``CONFIG_BT_MESH_PB_ADV`` configuration options must be enabled.

``mesh prov remote-adv <UUID(1-16 hex)> <NetKeyIdx> <Addr> <AttDur(s)>``
-----------------------------------------------------------------------------------

	Provisions a nearby device into the mesh. The mesh node starts scanning for unprovisioned
	...
	* ``UUID``: The UUID of the unprovisioned device. Providing a hex-string shorter than 16 bytes will populate the N most significant bytes of the array and zero-pad the rest.
	* ``NetKeyIdx``: The Index of the network key to pass to the device.
	* ``Addr``: The first unicast address to assign to the unprovisioned device. The device will occupy as many addresses as it has elements, and all must be available.
...
To allow a device to provision devices over GATT, the ``CONFIG_BT_MESH_PROVISIONER``
and ``CONFIG_BT_MESH_PB_GATT_CLIENT`` configuration options must be enabled.

``mesh prov remote-gatt <UUID(1-16 hex)> <NetKeyIdx> <Addr> <AttDur(s)>``
-------------------------------------------------------------------------

	Provisions a nearby device into the mesh. The mesh node starts scanning for connectable
	...
	* ``UUID``: The UUID of the unprovisioned device. Providing a hex-string shorter than 16 bytes will populate the N most significant bytes of the array and zero-pad the rest.
	* ``NetKeyIdx``: The Index of the network key to pass to the device.
	* ``Addr``: The first unicast address to assign to the unprovisioned device. The device will occupy the same number of addresses as it has elements, and all addresses must be available.
	* ``AttDur``: If supported, the unprovisioned device will identify its duration in seconds. For more details, see:ref:`bluetooth_mesh_models_health_srv_attention` for details.

``mesh prov uuid [UUID(1-16 hex)]``
-----------------------------------

	Gets or sets the mesh node's UUID, used in the unprovisioned beacons.

	* ``UUID``: If present, sets new 128-bit UUID value. Providing a hex-string shorter than 16 bytes will populate the N most significant bytes of the array and zero-pad the rest. 
    
    If omitted, prints the current UUID. To enable this command, the ``CONFIG_BT_MESH_SHELL_PROV_CTX_INSTANCE`` option must be enabled.


``mesh prov input-num <Number>``
--------------------------------

	Enter a numeric OOB (Out of Band) authentication value. This is only valid during configuration when prompted by the shell. 
	The entered number must match the number provided by other participants in the configuration.
...
--------------------------------
	...
	provisioning. The input string must match the string presented by other participants in
	the provisioning.
...
----------------------------------------
	...
	``CONFIG_BT_MESH_SHELL_PROV_CTX_INSTANCE`` option must be enabled.

	* ``Val``: If present, indicates the new hexadecimal value of the static OOB. Providing a hex-string shorter than 16 bytes will populate the N most significant bytes of the array and zero-pad the rest. 
	           If omitted, the static OOB value is cleared.
...
--------------------------------------------
	...
	* ``NetKeyIdx``:  The index of the network key to provision.
	* ``Addr``: The first unicast address to assign to the device. The device will occupy as many addresses as it has elements, and all must be available.
...
------------------------------------------
	...
	Enable or disable printing incoming unprovisioned beacons. allowing the provisioner device to detect nearby unprovisioned devices and provision them。
	To enable this command, the ``CONFIG_BT_MESH_SHELL_PROV_CTX_INSTANCE`` option must be enabled.

	* ``Val``: Whether to enable the unprovisioned beacon printing.

``mesh prov remote-pub-key <PubKey>``
-------------------------------------
	Provide Device the device's public key.

	* ``PubKey`` - The device's public key in big-endian numbers.
...
-----------------------------------------------
	...
	* ``Action`` - Input action. Allowed values:
		...
		* ``0`` - No input action.
		* ``1`` - Push action set.
		* ``2`` - Twist action set.( ** To be corrected ** )
		* ``4`` - Input number action set.
		* ``8`` - Input String action set.
...
------------------------------------------------
	...
	* ``Val`` - Static OOB value. Providing a hex-string shorter than 32 bytes will populate the N most significant bytes of the array and zero-pad the remaining bytes.
...
``CONFIG_BT_MESH_PROXY_CLIENT`` configuration option.

``mesh proxy connect <NetKeyIdx>``
----------------------------------

	Automatically connect a nearby proxy server to the mesh.

	* ``NetKeyIdx``: The index of the network key to connect.
...
-------------------------------------
	...
	* ``NetKeyIdx``: The index of the network key to disconnect.


``mesh proxy solicit <NetKeyIdx>``
----------------------------------

	Start a proxy request for a subnet, support for this feature can be enabled through the
	``CONFIG_BT_MESH_PROXY_SOLICITATION`` configuration option.

	* ``NetKeyIdx``: The index of the network key to send Solicitation PDUs to.
...
``CONFIG_BT_MESH_CFG_CLI`` configuration option. This is implemented as a separate
...
target net`` to target specific nodes. When configuring a Bluetooth Mesh shell node, it is assumed that
the ``CONFIG_BT_MESH_SHELL_PROV_CTX_INSTANCE`` option is enabled with the shell
...
* Configure the local device (``mesh prov local``).
* The shell module should now target itself.
* Monitor the composition data of the local node (``mesh models cfg get-comp``).
* Use Configuration Client commands to configure the local node as needed.
...
* Use Configuration Client commands to configure the node as needed.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Get and set the timeout used by the Configuration Client model during message sending.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``Identity``: If present, sets the Identity state of the node.

``mesh models cfg polltimeout-get <LPNAddr>``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

	Get current value of the PollTimeout timer for an LPN within a Friend node.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Add a network key to the target node. If enabled, adds the key to the configuration database.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``Addr``: The address of the element the model is on.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``ElemAddr``: The address of the element the model is on.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Retrieve or set the publication parameters for a model. If all publication parameters are
	included, they will become the new publication parameters of the model.  If all publication
	parameters are omitted, the current publication parameters for the model will be printed.
	...
	* ``CID``: If present, determines the Company ID of the model. If omitted, the model is a Bluetooth SIG defined model.
	...
	Publication parameters:
		...
		* ``PerRes``:  The resolution of the publication period steps:
		...
		* ``PerSteps``: The number of publication period steps, or 0 to disable periodic publication.
		* ``Count``: The number of retransmission for each published message (``0`` to ``7``).
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Set the publication parameters for a model with a virtual address.
	...
	* ``CID``: If present, determines the Company ID of the model. If omitted, the model is a Bluetooth SIG defined model.
	...
	Publication parameters:
		...
		* ``UUID``: The destination virtual address to publish to. Providing a hexadecimal string shorter than 16 bytes will fill the N most significant bytes of the array and pad the remaining bytes with zeros.
		* ``AppKeyIdx``: The application key index to use for publishing.
		* ``Cred``: Whether to use friendship credentials for publishing when acting as a low-power node.
		* ``TTL``: TTL value for the publish with (``0x00`` to ``0x07f``).
		* ``PerRes``: The resolution of the publication period steps:
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Subscription the model to a group address. The models only receive messages sent to their
	...
	* ``ElemAddr``: The address of the element the model is on.
	* ``SubAddr``: The 16-bit group address the model should subscribe to (``0xc000`` to ``0xFEFF``).
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``ElemAddr``: The address of the element the model is on.
	* ``SubAddr``: The 16-bit group address the model should remove from its subscription list (``0xc000`` to ``0xFEFF``).
	* ``MID``: The model ID of the model to add the subscription to.
	* ``CID``: If present, determines the Company ID of the model. If omitted, the model is a Bluetooth SIG defined model.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``ElemAddr``: The address of the element the model is on.
	* ``LabelUUID``: The 128-bit label UUID of the virtual address to subscribe to. Providing a hex-string shorter than 16 bytes will populate the N most significant bytes of the array and zero-pad the rest.
	* ``MID``: The model ID of the model to add the subscription to.
	* ``CID``: If present, determines the Company ID of the model. If omitted, the model is a Bluetooth SIG defined model.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``ElemAddr``: The address of the element where the model is located.
	* ``LabelUUID``: The 128-bit label UUID of the virtual address to remove the subscription of.  Providing a hex-string shorter than 16 bytes will populate the N most significant bytes of the array and zero-pad the rest.
	* ``MID``: The model ID of the model to add the subscription to.
	* ``CID``: If present, determines the Company ID of the model. If omitted, the model is a Bluetooth SIG defined model.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``ElemAddr``: The address of the element the model is on.
	* ``SubAddr``: The 16-bit group address the model should added to the subscription list (``0xc000`` to ``0xFEFF``).
	* ``MID``: The model ID of the model to add the subscription to.
	* ``CID``: If present, determines the Company ID of the model. If omitted, the model is a Bluetooth SIG defined model.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``ElemAddr``: The address of the element the model is on.
	* ``LabelUUID``: The 128-bit label UUID of the virtual address as the new Address to be added to the subscription list. Providing a hex-string shorter than 16 bytes will populate the N most significant bytes of the array and zero-pad the rest.
	* ``MID``: The model ID of the model to add the subscription to.
	* ``CID``: If present, determines the Company ID of the model. If omitted, the model is a Bluetooth SIG defined model.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Delete all group and virtual address subscriptions from for model.

	* ``ElemAddr``: The address of the element the model is located.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Retrieve the address list for model subscriptions.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``Phase``: The new Key Refresh Phase. Valid phases are:
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Retrieve or set the heartbeat subscription parameters. A node only receives heartbeat messages that match its heartbeat subscription parameters.

    Set the heartbeat subscription parameters if present, or print the current heartbeat subscription parameters if called without parameters.

	* ``Src``:  The unicast source address to receive Heartbeat messages from.
	* ``Dst``: The destination address to receive Heartbeat messages on.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Retrieve or set the heartbeat publication parameters. Set the heartbeat publication parameters if present, 
	or print the current heartbeat publication parameters if called without parameters.

	* ``Dst``: The destination address to publish heartbeat messages to.
	...
	* ``Features``: Bit field of features that should trigger a Heartbeat publication when changed:
	...
	* ``NetKeyIdx``: The index of the network key used to publish heartbeat messages.
...
``CONFIG_BT_MESH_HEALTH_CLI`` configuration option. It is implemented as a separate
...
using the Health Client commands. To select a spesific Health Client instance the user can utilize
the commands ``mesh models health instance set`` and ``mesh models health instance get-all``.

The Health Client can use the general messages parameters set by ``mesh target dst``,
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Set the Health Client model instance to be used.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Print all available Health Client model instances on the device.

``mesh models health fault-get <CID>``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

	Retrieve a list of registered faults for a Company ID.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Invoke a self-test procedure, and displays the list of triggered faults.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Set the current Health Server publication period divisor. 
	When a fault is detected, the Health Server will start publishing fault status at shortened intervals. 
	The shortened interval is determined by the Health Server publication period divisor: 
	Fault publication period = Publication period / 2^divisor.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Set the current Health Server publication period divisor without requesting a response. 
	When a fault is detected, the Health Server will start publishing fault status at shortened intervals. T
	he shortened interval is determined by the Health Server publication period divisor: 
	Fault publication period = Publication period / 2^divisor.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Enable the Health Server attention state for a period of time.

	* ``Time``: The duration of the attention state in seconds (0 to 255).


``mesh models health attention-set-unack <Time(s)>``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

	Enable the Health Server attention state for a period of time without requesting a response.
...
``CONFIG_BT_MESH_BLOB_CLI`` option, and disabling the
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``Addr``: The unicast address of the BLOB Transfer Server model of the target node.


``mesh models blob cli bounds [<Group>]``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

	Retrieve the total bounds parameters for all target nodes.

	* ``Group``: An optional group address to use when communicating with target nodes. If omitted, the BLOB Transfer Client will address each target node individually


``mesh models blob cli tx <Id> <Size> <BlockSizeLog> <ChunkSize> [<Group> [<Mode(push, pull)>]]``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

	Executes a BLOB transfer to the target nodes. The BLOB Transfer Client will send a virtual BLOB 
	to all target nodes and then publish a message when the transfer is complete. 
	Note that all target nodes must first be configured to receive transfers using the ``mesh models blob srv rx``
	command.

	* ``Id``: A 64-bit BLOB transfer ID.
	* ``Size``: The size of the BLOB in bytes.
	* ``BlockSizeLog``:The logarithmic representation of the BLOB's block size. The final block size will be ``1 << block size log`` bytes.
	* ``ChunkSize``:The Chunk size in bytes.
	* ``Group``:  An Optional group address to use when communicating with Target nodes. If omitted or set to 0, the BLOB Transfer Client will address each Target node individually.
	* ``Mode``: The BLOB transfer mode to use. Must be either ``push`` (Push BLOB Transfer Mode) or ``pull`` (Pull BLOB Transfer Mode). If omitted, ``push`` will be used by default.
...
``CONFIG_BT_MESH_BLOB_SRV`` option. The BLOB Transfer Server model is capable of
...
``CONFIG_BT_MESH_BLOB_CLI`` and ``CONFIG_BT_MESH_DFU_CLI``. The Firmware
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Adds a virtual DFU image slot that can be transferred as a DFU image. The image slot will be
	...
	* ``Size``:The size of the DFU image slot in bytes.
	* ``FwID``: The firmware ID in hexadecimal format.
...
``CONFIG_BT_MESH_BLOB_SRV`` and ``CONFIG_BT_MESH_DFU_SRV``. The Firmware
...
``CONFIG_BT_MESH_DFD_SRV`` configuration option. The shell commands for this model
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Create the header for synthetic data page 0.
...
``CONFIG_BT_MESH_SAR_CFG_CLI`` configuration option. The SAR Configuration Client
...
^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Send SAR configuration to the transmitter to obtain the message.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Transmit the setup message for SAR configuration to the emitter.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Send the SAR configuration to the receiver to obtain the message..
...
``CONFIG_BT_MESH_PRIV_BEACON_CLI`` configuration option.

``mesh models prb priv-beacon-get``
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

	Obtain the dedicated beacon status of the target. Possible value translations:
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	 ...
	 Set the dedicated beacon status of a target.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	 ...
	 obtain the private GATT proxy status of a target, possible values are:
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Set the private GATT proxy status of a target.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	 ...
	 Obtain the private node ID status of a target, possible values are:
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Set the private node ID status of a target.
...
``CONFIG_BT_MESH_OP_AGG_CLI`` configuration option. The Opcodes Aggregator Client
...
``CONFIG_BT_MESH_RPR_CLI`` configuration option. The Remote Provisioning Client model
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``UUID``: The UUID of the device to scan for. Providing a hex string shorter than 16 bytes will pad the array with the N most significant bytes and zero-fill the rest. If omitted, all devices will be reported.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	* ``UUID``: The UUID of the device to start extended scanning for. Providing a hex string shorter than 16 bytes will pad the array with the N most significant bytes and zero-fill the rest.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Start extended scanning for Remote Provisioning Server.

	* ``ADType``: A List of AD types to include in scan reports. Must contain 1 to ``CONFIG_BT_MESH_RPR_AD_TYPES_MAX`` entries.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Configure a mesh node using the PB-Remote Provisioning Bearer.
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
	...
	Re-configure a mesh node using the PB-Remote Provisioning Bearer.
...
``CONFIG_BT_MESH_CDB`` configuration option. The Configuration database is only
...
The On-Demand Private GATT Proxy Client model is an optional subsystem within a mesh, 
which can be enabled through the configuration option``CONFIG_BT_MESH_OD_PRIV_PROXY_CLI``.

``mesh models od_priv_proxy od-priv-gatt-proxy [Dur(s)]``
---------------------------------------------------------

	Set the state of the On-Demand Private GATT Proxy on an active destination, or retrieve the value of this state from it..

	* ``Dur``:  If provided, set the state of the On-Demand Private GATT Proxy to this value (in seconds). If not provided, retrieve the current value.
...
``CONFIG_BT_MESH_SOL_PDU_RPL_CLI`` configuration option.

``mesh models sol_pdu_rpl sol-pdu-rpl-clear <RngStart> <Ackd> [RngLen]``
------------------------------------------------------------------------

	This command clears the Solicitation Replay Protection List (SRPL) for active destinations within a given Solicitation Source (SSRC) address range.
...
-----------------
	...
	Retrieve frame statistics. This command prints the number of frames received, 
	as well as the number of scheduled and successful transmission attempts.


``mesh stat clear``
-------------------

	Clear all previously collected statistics.
