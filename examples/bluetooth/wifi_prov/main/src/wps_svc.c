...
static void nus_ccc_cfg_changed(const struct bt_gatt_attr *attr, uint16_t value)
{
    if (wps_cb.send_enabled) {
        wm_log_debug("Indication has been turned %s", value == BT_GATT_CCC_INDICATE ? "on" : "off");
...
static void on_sent(struct bt_conn *conn, struct bt_gatt_indicate_params *params, uint8_t err)
{
    ...
    if (err) {
        wm_log_error("Data send, conn %p", (void *)conn);
        return;
    }
    if (wps_cb.sent) {
        wps_cb.sent(conn);
...
static bool le_param_req(struct bt_conn *conn, struct bt_le_conn_param *param)
{
    ...
    bt_addr_le_to_str(bt_conn_get_dst(conn), addr, sizeof(addr));
    wm_log_debug("LE conn param req: %s int (0x%04x, 0x%04x) lat %d to %d", addr, param->interval_min, param->interval_max,
...
static void le_param_updated(struct bt_conn *conn, uint16_t interval, uint16_t latency, uint16_t timeout)
{
    ...
    bt_addr_le_to_str(bt_conn_get_dst(conn), addr, sizeof(addr));
    wm_log_debug("LE conn param updated: %s int 0x%04x lat %d to %d", addr, interval, latency, timeout);
}
...
static void security_changed(struct bt_conn *conn, bt_security_t level, enum bt_security_err err)
{
    ...
    if (!err) {
        wm_log_debug("Security changed: %s level %u", addr, level);
    } else {
        wm_log_warn("Security failed: %s level %u err %d", addr, level, err);
    }
}
#endif
...
static void le_phy_updated(struct bt_conn *conn, struct bt_conn_le_phy_info *param)
{
    ...
    wm_log_debug("LE PHY Updated: %s Tx 0x%x, Rx 0x%x", addr, param->tx_phy, param->rx_phy);
}
...
static void le_data_len_updated(struct bt_conn *conn, struct bt_conn_le_data_len_info *info)
{
    ...
    wm_log_debug("Data length updated: %s max tx %u (%u us) max rx %u (%u us)", addr, info->tx_max_len, info->tx_max_time,
...
int bt_wps_init(struct bt_svc_cb *callbacks)
{
    ...
    if (err) {
        wm_log_warn("Advertising failed to start (err %d)", err);
        return 0;
    }
    ble_wps_state = WPS_BLE_ADVERTISING;
    ...
    wm_log_info("WiFi provision service ready");
    return 0;
}
...
{
    if (ble_wps_state == WPS_BLE_ADVERTISING) {
        wm_log_debug("wps stop ble advertising");
        bt_le_adv_stop();

    } else if (ble_wps_state == WPS_BLE_CONNECTED) {
        wm_log_debug("wps disconnect with remote device");
        ble_wps_exit = true;
