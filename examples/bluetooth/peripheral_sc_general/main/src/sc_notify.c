                          ...
                          uint8_t flags)
{
    ...
    wm_log_dump(WM_LOG_LEVEL_DEBUG, "Receive data", 32, buf, len);
    return len;
}
...
int app_send_via_notify(const uint8_t *data, uint16_t len)
{
    ...
    params.attr = &notify_svc.attrs[3];
    ;
    params.data = data;
    params.len  = len;
...
int run_notify_sample(void)
{
    ...
    while (1) {
        ...
        while (1) {
            /**notify dummy data*/
            err = app_send_via_notify(access_value, bt_gatt_get_mtu(current_conn) - 7);
            access_value[0]++;
            ...
            k_sem_take(&app_sem, K_MSEC(100));
            k_sem_take(&app_sem, K_MSEC(10));
        }
    }
