...
extern int jobs_demo_main(int argc, char *argv[]);
...
static void aws_task(void *pvParameters)
{
    if (wm_netif_get_netif(WM_NETIF_TYPE_WIFI_STA) == NULL) {
        ...
        jobs_demo_main(0, NULL);
    }

    vTaskDelete(NULL);
}

static void aws_job(int argc, char *argv[])
{
    ...
    xTaskCreate(aws_task, "aws_task", AWS_MQTT_CLI_TASK_STACK_SIZE, NULL, AWS_MQTT_CLI_TASK_PRIORITY, &task_handle);
}

WM_CLI_CMD_DEFINE(aws_job, aws_job, aws job demo, aws_job); //cppcheck # [syntaxError]
