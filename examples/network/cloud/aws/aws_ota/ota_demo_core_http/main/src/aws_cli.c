...
extern int ota_demo_core_http(int argc, char **argv);
...
static void aws_task(void *pvParameters)
{
    if (wm_netif_get_netif(WM_NETIF_TYPE_WIFI_STA) == NULL) {
        ...
        ota_demo_core_http(0, NULL);
    }

    vTaskDelete(NULL);
}

static void aws_ota(int argc, char *argv[])
{
...
WM_CLI_CMD_DEFINE(aws_ota, aws_ota, aws ota demo, aws_ota); //cppcheck # [syntaxError]
