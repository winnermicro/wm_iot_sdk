...
{
    static wm_aws_shadow_param_t aws_param = {
        .update_fun = aws_device_update,
    };

    if (wm_netif_get_netif(WM_NETIF_TYPE_WIFI_STA) == NULL) {
        wm_log_error("network is not ready, please start wifi connection");
    } else {
        shadow_demo_main(&aws_param);
    }

    vTaskDelete(NULL);
}

static void aws_led(int argc, char *argv[])
{
    ...
    aws_device_init();
    xTaskCreate(aws_task, "aws_cli", AWS_MQTT_CLI_TASK_STACK_SIZE, NULL, AWS_MQTT_CLI_TASK_PRIORITY, &task_handle);
}

WM_CLI_CMD_DEFINE(aws_led, aws_led, aws led demo, aws_led); //cppcheck # [syntaxError]
