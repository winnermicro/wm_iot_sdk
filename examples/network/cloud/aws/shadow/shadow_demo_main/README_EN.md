...
This example is developed based on the official AWS example. Its core objective is to demonstrate how devices can use the MQTTS protocol to access the AWS platform and achieve the functions of reporting local parameter information and receiving messages sent from the platform by subscribing to shadow - related topics. It specifically covers the following aspects:

1. The device successfully establishes a connection to the AWS server by virtue of the MQTTS protocol.
2. The device publishes an empty message to AWS with the topic "$aws/things/THING_NAME/shadow/delete/accepted". Here, THING_NAME represents the item name, and the meaning of THING_NAME mentioned subsequently is the same. This operation aims to delete the shadow device, preparing for subsequent operations.
3. The device publishes another message to AWS. This time, the topic is "$aws/things/THING_NAME/shadow/update", which is used to create a shadow device. The message format is predefined in SHADOW_DESIRED_JSON. Users can flexibly modify it according to the requirements of actual application scenarios to meet different business logics.
4. This example is built based on the official AWS example and is mainly used to demonstrate how devices can access the AWS platform via the HTTPS protocol. It specifically achieves the following functions:
...
Example Location：`examples/network/cloud/aws/shadow/shadow_demo_main`
...
4. aws_shadow: Start the AWS demo.
