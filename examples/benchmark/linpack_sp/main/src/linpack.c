...
int row;
{
    fprintf(stderr, "%11.2f%11.2f%11.2f%11.0f%11.2f%11.2f\n", (double)timer[0][row], (double)timer[1][row],
            (double)timer[2][row], (double)timer[3][row], (double)timer[4][row], (double)timer[5][row]);
    return 0;
}
...
/*----------------------*/
void dgefa(a, lda, n, ipvt, info) REAL a[];
int lda, n, ipvt[], *info;
...

{
    ...
    *info = 0;
    nm1   = n - 1;  //cppcheck # [negativeIndex]
    if (nm1 >= 0) { //cppcheck # [negativeIndex]
        for (k = 0; k < nm1; k++) {
            kp1 = k + 1;
    ...
    ipvt[n - 1] = n - 1; //cppcheck # [negativeIndex]
...
void dgesl(a, lda, n, ipvt, b, job) int lda, n, ipvt[], job;
REAL a[], b[];
...
/*----------------------*/
void dmxpy(n1, y, n2, ldm, x, m) REAL y[], x[], m[];
int n1, n2, ldm;
...
*/
{
    ...
    if (j >= 4) {
        j = j - 1;
        for (i = 0; i < n1; i++)
            y[i] = ((((y[i]) + x[j - 3] * m[ldm * (j - 3) + i]) + x[j - 2] * m[ldm * (j - 2) + i]) +
                    x[j - 1] * m[ldm * (j - 1) + i]) +
                   x[j] * m[ldm * j + i];
    }
    ...
    if (j >= 8) {
        j = j - 1;
        for (i = 0; i < n1; i++)
            y[i] = ((((((((y[i]) + x[j - 7] * m[ldm * (j - 7) + i]) + x[j - 6] * m[ldm * (j - 6) + i]) +
                        x[j - 5] * m[ldm * (j - 5) + i]) +
                       x[j - 4] * m[ldm * (j - 4) + i]) +
                      x[j - 3] * m[ldm * (j - 3) + i]) +
                     x[j - 2] * m[ldm * (j - 2) + i]) +
    ...
    for (j = jmin - 1; j < n2; j = j + 16) {
        for (i = 0; i < n1; i++)
            y[i] = ((((((((((((((((y[i]) + x[j - 15] * m[ldm * (j - 15) + i]) + x[j - 14] * m[ldm * (j - 14) + i]) +
                                x[j - 13] * m[ldm * (j - 13) + i]) +
                               x[j - 12] * m[ldm * (j - 12) + i]) +
                              x[j - 11] * m[ldm * (j - 11) + i]) +
                             x[j - 10] * m[ldm * (j - 10) + i]) +
...
#include <sys/resource.h>
    ...
    t = (REAL)(ru.ru_utime.tv_sec + ru.ru_stime.tv_sec) + ((REAL)(ru.ru_utime.tv_usec + ru.ru_stime.tv_usec)) / 1.0e6;
    return t;
}
...
double floor(double x)
{
    ...
    int32_t j0 = ((i0 >> 52) & 0x7ff) - 0x3ff; //cppcheck # [shiftNegativeLHS]
...
void benchmark_linpack_main()
{
    static REAL aa[120][120], a[120][121], b[120], x[120];
    REAL cray, ops, total, norma, normx;
    REAL resid, residn, eps, t1, tm, tm2;
    REAL epslon(), second(), kf;
    static int ipvt[120], n, i, ntimes, info, lda, ldaa, kflops;
    ...
    fprintf(stdout, PREC);
    fprintf(stdout, "Precision Linpack\n\n");
    fprintf(stderr, ROLLING);
    fprintf(stderr, PREC);
    fprintf(stderr, "Precision Linpack\n\n");
    ...
    printf("  %8.1f      %16.8e%16.8e%16.8e%16.8e\n", (double)residn, (double)resid, (double)eps, (double)x[0] - 1,
           (double)x[n - 1] - 1);
