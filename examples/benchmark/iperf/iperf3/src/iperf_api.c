...
iperf_parse_hostname(struct iperf_test *test, char *spec, char **p, char **p1) {
    ...
    if ((*p = strtok(spec, "%")) != NULL &&
        ...
        if (inet_pton(AF_INET6, *p, &ipv6_addr) == 1 &&
            ip6_addr_isallnodes_linklocal((ip6_addr_t *)(&ipv6_addr))) {
            if (test->debug) {
...
iperf_exchange_results(struct iperf_test *test)
{
    ...
    tv.tv_sec = 3;
    tv.tv_usec = 0;
    setsockopt(test->ctrl_sck, SOL_SOCKET, SO_RCVTIMEO, (struct timeval *)&tv, sizeof(struct timeval));
#endif
#endif
...
get_parameters(struct iperf_test *test)
{
	...
	if ((j_p = cJSON_GetObjectItem(j, "len")) != NULL) {
	    if (j_p->valueint >= (128 * 1024)) {
	        test->settings->blksize = 10 * 1024;
	        printf("length of buffer (%d) maybe too large from get parameters, \n"
	               "you can use -l set buffer size, current reset to %d\n", j_p->valueint, test->settings->blksize);
	    } else {
...
get_results(struct iperf_test *test)
{
    ...
    int sid;
    int64_t cerror, pcount, omitted_cerror = 0, omitted_pcount = 0;
    double jitter;
    iperf_size_t bytes_transferred;
    int retransmits;
    struct iperf_stream *sp;
	...
	if (j_cpu_util_total == NULL || j_cpu_util_user == NULL || j_cpu_util_system == NULL || j_sender_has_retransmits == NULL) {
		...
		for (i=0; i<n; ++i) {
			...
			if (j_id == NULL || j_bytes == NULL || j_retransmits == NULL || j_jitter == NULL || j_errors == NULL || j_packets == NULL) {
			    ...
			    sid = j_id->valueint;
			    bytes_transferred = (iperf_size_t)j_bytes->valuedouble;
...
iperf_defaults(struct iperf_test *testp)
{
    ...
    //testp->debug = 1;
    //testp->debug_level = DEBUG_LEVEL_MAX;
...
/**************************************************************************/
void iperf_free_all_stream(struct iperf_test *test)
{
    if (!test) {
        return;
    }
    ...
    if (test->bitrate_limit_intervals_traffic_bytes != NULL) {
        ...
        test->bitrate_limit_intervals_traffic_bytes = NULL;
    }
}
                ...
                if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                    ...
                    if (test->json_output) {
                        ...
                        if (test->role == 's' && !sp->sender) {
                            ...
                            const char wm_report_bw_udp_format[] = "[%3d]%s %6.2f-%-6.2f sec  %ss  %ss/sec  %5.3f ms   %d/%" PRIu64 " (%.2g%%)  %s\n";
                            iperf_printf(test, wm_report_bw_udp_format, sp->socket, mbuf, start_time, sender_time, ubuf, nbuf, 0.0, 0, (sender_packet_count - sender_omitted_packet_count), (double) 0, report_sender);
                        }
