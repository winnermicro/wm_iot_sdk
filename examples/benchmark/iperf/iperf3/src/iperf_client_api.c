...
iperf_run_client(struct iperf_test * test)
{
	...
	if (test->state != TEST_RUNNING) {
		struct iperf_stream *sp;
		SLIST_FOREACH(sp, &test->streams, streams) {
			if (FD_ISSET(sp->socket, &write_set)) {
...
#if 0
    if (timeout && !timeout->tv_sec && timeout->tv_usec < DEAULT_CLINET_RUN_TIMEOUT) {
        timeout->tv_usec = DEAULT_CLINET_RUN_TIMEOUT;
    }
	#if 1
	else if (!timeout) {
        used_timeout.tv_sec = 0;
        used_timeout.tv_usec = DEAULT_CLINET_RUN_TIMEOUT;
        timeout = &used_timeout;
    }
	#endif
#endif
