...
#endif
    printf("Memory location  : %s\n", MEM_LOCATION); //cppcheck # [invalidPrintfArgType_s]
...
#if HAS_FLOAT
        if (known_id == 3) {
            printf("CoreMark 1.0 : %f / %s %s", default_num_contexts * results[0].iterations / time_in_secs(total_time),
            ...
            printf("Score (Coremarks/MHz): %6.2f\n",
                   default_num_contexts * results[0].iterations / time_in_secs(total_time) /
                       wm_drv_rcc_get_config_clock(wm_dt_get_device_by_name("rcc"), WM_RCC_TYPE_CPU));
        }
#endif
    }
    if (total_errors > 0)
