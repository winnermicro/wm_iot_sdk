...
static __inline short ClipToShort6(int x)
{
#if 1 
	short ret;
	asm ("clipi.s32 %0,%1,16\n\t"
             "asri %0,%0,6\n\t"
             :"=r"(ret):"r"(x));
        return (short)ret;
#else
	asm ("ssat %0,%0,16,ASR,6":"+r"(x));
        return (short)x;
#endif
}
#define MC0M(x)	{ \
...
static __inline int dext_20(__int64 a)
{
	int ret;
	asm("dexti %0, %1, %R1, 20\n\t":"=r"(ret):"r"(a));
...
//	*(pcm + 0) = ClipToShort6((int)SAR64(sum1L, (32-CSHIFT)));
	*(pcm + 0) = ClipToShort6(dext_20(sum1L));
//	*(pcm + 0) = ClipToShort(dext_20(sum1L), DEF_NFRACBITS);
...
//        *(pcm + 16) = ClipToShort6((int)SAR64(sum1L, (32-CSHIFT)));
	*(pcm + 16) = ClipToShort6(dext_20(sum1L));
//	*(pcm + 16) = ClipToShort(dext_20(sum1L), DEF_NFRACBITS);
...
//	*(pcm + 0) = ClipToShort6((int)SAR64(sum1L, (32-CSHIFT)));
//        *(pcm + 1) = ClipToShort6((int)SAR64(sum1R, (32-CSHIFT)));
      *(pcm + 0) = ClipToShort6(dext_20(sum1L));
      *(pcm + 1) = ClipToShort6(dext_20(sum1R));

//	*(pcm + 0) = ClipToShort(dext_20(sum1L), DEF_NFRACBITS);
//	*(pcm + 1) = ClipToShort(dext_20(sum1R), DEF_NFRACBITS);
...
//	*(pcm + 2*16 + 0) = ClipToShort6((int)SAR64(sum1L, (32-CSHIFT)));
//	*(pcm + 2*16 + 1) = ClipToShort6((int)SAR64(sum1R, (32-CSHIFT)));

      *(pcm + 2*16 + 0) = ClipToShort6(dext_20(sum1L));
      *(pcm + 2*16 + 1) = ClipToShort6(dext_20(sum1R));

//	*(pcm + 2*16 + 0) = ClipToShort(dext_20(sum1L), DEF_NFRACBITS);
//	*(pcm + 2*16 + 1) = ClipToShort(dext_20(sum1R), DEF_NFRACBITS);
...
//		*(pcm + 0)         = ClipToShort6((int)SAR64(sum1L, (32-CSHIFT)));
//		*(pcm + 1)         = ClipToShort6((int)SAR64(sum1R, (32-CSHIFT)));
//		*(pcm + 2*2*i + 0) = ClipToShort6((int)SAR64(sum2L, (32-CSHIFT)));
//		*(pcm + 2*2*i + 1) = ClipToShort6((int)SAR64(sum2R, (32-CSHIFT)));
                *(pcm + 0)         = ClipToShort6(dext_20(sum1L));
                *(pcm + 1)         = ClipToShort6(dext_20(sum1R));
                *(pcm + 2*2*i + 0) = ClipToShort6(dext_20(sum2L));
                *(pcm + 2*2*i + 1) = ClipToShort6(dext_20(sum2R));

	//	*(pcm + 0)         = ClipToShort(dext_20(sum1L), DEF_NFRACBITS);
	//	*(pcm + 1)         = ClipToShort(dext_20(sum1R), DEF_NFRACBITS);
	//	*(pcm + 2*2*i + 0) = ClipToShort(dext_20(sum2L), DEF_NFRACBITS);
	//	*(pcm + 2*2*i + 1) = ClipToShort(dext_20(sum2R), DEF_NFRACBITS);
		pcm += 2;
	}
}
