...
static int lcd_demo_clean_screen(wm_device_t *dev, uint8_t *buf, uint32_t buf_len, uint16_t bk_color)
{
...
static int lcd_demo_show_image(wm_device_t *dev, uint8_t *buf, uint32_t buf_len, image_attr_t img)
{
    wm_lcd_data_desc_t data_desc = { 0 };
    uint16_t width               = 0;
    uint16_t high                = 0;
...
static void wm_lcd_demo(void *arg)
{
    ...
    while (1) {
        ...
        ret = lcd_demo_clean_screen(dev, app_buf, block_size, LCD_RGB565_BLUE);
        if (ret != WM_ERR_SUCCESS) {
            wm_log_info("wm_lcd_demo_show_image ret=%d", ret);
        }
        ...
        ret = lcd_demo_show_image(dev, app_buf, block_size, img);
        if (ret != WM_ERR_SUCCESS) {
            wm_log_info("wm_lcd_demo_show_image ret=%d", ret);
        }
        vTaskDelay(pdMS_TO_TICKS(2000));
