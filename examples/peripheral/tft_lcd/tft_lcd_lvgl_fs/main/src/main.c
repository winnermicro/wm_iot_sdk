...
/* the path of image in file system.
 * the first letter 'D' in the path must equals to the define of CONFIG_LV_FS_FATFS_LETTER  
 * also need equals to the define of CONFIG_FATFS_INTERNAL_FLASH_DISK_FF_VOLUME_STRS
 */
const char *app_logo_fs_path = "D:assets/winner_micro_logo_120x120.png";

extern int app_check_logo_in_fs(const char *logo_path);
#endif
#endif
...
#if CONFIG_COMPONENT_FATFS_ENABLED
    /* try to get the logo picture from fatfs partition in the flash */
    if (app_check_logo_in_fs(app_logo_fs_path) == WM_ERR_SUCCESS) {
        memcpy(src_image, app_logo_fs_path, strlen(app_logo_fs_path));
        path = src_image;
    }
#endif

    /*if no valid logo picture in filesystem, use the default on from array */
    if (path == NULL) {
        wm_log_info("show logo from array");
        path = (char *)&winner_micro_logo_120x120;
    } else {
        wm_log_info("show logo from filesystem");
    }
