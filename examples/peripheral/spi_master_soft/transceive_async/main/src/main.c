...
static void example_spim_async_callback(int result, void *data)
{
    wm_log_info("enter example_spim_async_callback, result:%d", result);
...
void example_spim_transceive_async(wm_device_t *dev)
{
    ...
    if (spim_trx_sem != NULL && tx_buf != NULL && rx_buf != NULL) {
        ...
        wm_drv_spim_transceive_async(dev, &config, &desc, example_spim_async_callback, NULL);
        ...
        wm_log_error("buffer or semaphore create fail");
    }
...
int main(void)
{
    ...
    wm_log_info("spi soft master async transecie start.");
