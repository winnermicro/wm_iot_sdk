...
#define WM_GDC0689_REFRESH_DELAY_MS (pdMS_TO_TICKS(500))
#define WM_GDC0689_DISPLAY_COUNT    (5)

static void gdc0689_demo(wm_device_t *dev)
{
    int ret = WM_ERR_SUCCESS;

    for (int i = 0; i < WM_GDC0689_DISPLAY_COUNT; i++) {
        wm_log_info("Displaying all segments.");
        ret = wm_gdc0689_display_all(dev);
        if (ret != WM_ERR_SUCCESS) {
            wm_log_error("Failed to display all, error code: %d.", ret);
            return;
        }
        vTaskDelay(WM_GDC0689_REFRESH_DELAY_MS);
