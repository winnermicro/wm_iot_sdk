...
{
#ifdef CONFIG_WPA_SUPPLICANT_ENABLE_DEBUG
    if (level >= wpa_debug_level) {
        int wpa_level = 0;
        va_list list;

        va_start(list, fmt);
        wpa_level = (level == MSG_ERROR   ? WM_LOG_LEVEL_ERROR :
                     level == MSG_WARNING ? WM_LOG_LEVEL_WARN :
                     level == MSG_INFO    ? WM_LOG_LEVEL_INFO :
                     level == MSG_DEBUG   ? WM_LOG_LEVEL_DEBUG :
                                            WM_LOG_LEVEL_VERBOSE);

        wm_log_vprintf(wpa_level, LOG_TAG, fmt, list);
        wm_log_printf(wpa_level, LOG_TAG, "\r\n");
        va_end(list);
    }
#endif
}
...
static void _wpa_hexdump(int level, const char *title, const u8 *buf, size_t len, int show, int only_syslog)
{
    ...
    wm_log_printf(wpa_level, LOG_TAG, "%s - hexdump(len=%lu):", title, (unsigned long)len);
    if (buf == NULL) {
        wm_log_printf(wpa_level, LOG_TAG, " [NULL]");
    } else if (show) {
        for (i = 0; i < len; i++)
            wm_log_printf(wpa_level, LOG_TAG, " %02x", buf[i]);
    } else {
        wm_log_printf(wpa_level, LOG_TAG, " [REMOVED]");
    }
    wm_log_printf(wpa_level, LOG_TAG, "\n");
}
...
#ifdef CONFIG_WPA_SUPPLICANT_ENABLE_DEBUG_SHOW_KEYS
                 1
#else
                 0
#endif
...
static void _wpa_hexdump_ascii(int level, const char *title, const void *buf, size_t len, int show)
{
    ...
    if (!show) {
        wm_log_printf(wpa_level, LOG_TAG, "%s - hexdump_ascii(len=%lu): [REMOVED]\n", title, (unsigned long)len);
        return;
    }
    if (buf == NULL) {
        wm_log_printf(wpa_level, LOG_TAG, "%s - hexdump_ascii(len=%lu): [NULL]\n", title, (unsigned long)len);
        return;
    }
    wm_log_printf(wpa_level, LOG_TAG, "%s - hexdump_ascii(len=%lu):\n", title, (unsigned long)len);
    while (len) {
        llen = len > line_len ? line_len : len;
        wm_log_printf(wpa_level, LOG_TAG, "    ");
        for (i = 0; i < llen; i++)
            wm_log_printf(wpa_level, LOG_TAG, " %02x", pos[i]);
        for (i = llen; i < line_len; i++)
            wm_log_printf(wpa_level, LOG_TAG, "   ");
        wm_log_printf(wpa_level, LOG_TAG, "   ");
        for (i = 0; i < llen; i++) {
            if (isprint(pos[i]))
                wm_log_printf(wpa_level, LOG_TAG, "%c", pos[i]);
            else
                wm_log_printf(wpa_level, LOG_TAG, "_");
        }
        for (i = llen; i < line_len; i++)
            wm_log_printf(wpa_level, LOG_TAG, " ");
        wm_log_printf(wpa_level, LOG_TAG, "\n");
