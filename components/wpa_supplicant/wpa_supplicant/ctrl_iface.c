...
#endif /* mask end for MBO */
#ifdef CONFIG_MBO /* CONFIG_WM_WFA_VERSION */
	if (0) {
		;
...
static int wpa_supplicant_ctrl_iface_set_mac(struct wpa_supplicant *wpa_s, const char *cmd)
{
    u8 addr[ETH_ALEN];

    if (wm_hex_str_to_array((char *)cmd, addr, ETH_ALEN) < 0)
        return -1;
...
#if 0
static int __wpa_supplicant_ctrl_iface_get_mac(struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
    return os_snprintf(buf, buflen, "%02x:%02x:%02x:%02x:%02x:%02x", wpa_s->own_addr[0],
                       wpa_s->own_addr[1], wpa_s->own_addr[2], wpa_s->own_addr[3], 
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (wpas_ctrl_iface_send_dscp_query(wpa_s, buf + 11))
			reply_len = -1;
#endif
	} else if (os_strncmp(buf, "SET_MAC ", 8) == 0) {
		if (wpa_supplicant_ctrl_iface_set_mac(wpa_s, buf + 8) < 0)
		    reply_len = -1;
#if 0
    } else if (os_strncmp(buf, "GET_MAC ", 8) == 0) {
		reply_len = __wpa_supplicant_ctrl_iface_get_mac(wpa_s, reply, reply_size);
#endif
	} else {
...
#ifdef CONFIG_WPA_SUPPLICANT_WFA_VERSION
