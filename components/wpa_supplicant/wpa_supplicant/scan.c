...
static void wpa_supplicant_scan(void *eloop_ctx, void *timeout_ctx)
{
    ...
    if (NORMAL_SCAN_REQ == wpa_s->last_scan_req) {
        ssid = wpa_s->conf->ssid;
...
ssid_list_set:
    ...
    else if (MANUAL_SCAN_REQ != wpa_s->last_scan_req) {
        if (wpa_s->conf && wpa_s->conf->ssid && !is_zero_ether_addr(wpa_s->conf->ssid->bssid))
            params.bssid = wpa_s->conf->ssid->bssid;
    }
#endif
