			       ...
			       const u8 *auth_frame, size_t len)
{
	...
	if (le_to_host16(header->u.auth.auth_alg) == WLAN_AUTH_SAE) {
...
#ifdef CONFIG_PMKSA_CACHE_SIMPLE
        pmksa_cache_sample_save(wpa_s, wpa_s->sme.sae.pmk, wpa_s->sme.sae.pmkid);
#endif
#ifdef CONFIG_WM_SME_OPT
	} else {
        ...
        if (mgmt)
        {
            ...
            wpa_drv_send_mlme(wpa_s, (u8 *)mgmt, mlen, 1, 0, 0);
            os_free(mgmt);
        }
