...
#ifdef CONFIG_WM_WFA_VERSION
static void wpa_bss_add_update(struct wpa_supplicant *wpa_s,
		const u8 *ssid,
		struct wpa_scan_res *res,
		struct os_reltime *fetch_time)
{
	...
	if (bss == NULL) {
		...
		wpa_msg(wpa_s, MSG_INFO, "Add bss" MACSTR "@%s.", MAC2STR(res->bssid), ssid + 2);
	} else {
...
static int wpa_nontx_bssid_get(uint8_t bssid_index,
		uint8_t max_bssid_ind, uint8_t const *ref_bssid, uint8_t *bssid)
{
	uint8_t B;
	uint16_t M;
	...
	os_memcpy(bssid, ref_bssid, ETH_ALEN);

	M = 1U << max_bssid_ind;

	B = bssid[5] % M;

	bssid[5] = bssid[5] - B + (B + bssid_index) % M;

	return 0;
}

static void wpa_mbssid_ie_parse(struct wpa_supplicant *wpa_s,
		struct wpa_scan_res *res,
		const uint8_t *mie, size_t mie_len)
{
	int max_bssid_ind;
	int res_size;

	struct wpa_scan_res *res_new;
	struct os_reltime fetch_time;

	uint8_t bssid[ETH_ALEN];

	uint8_t *sub_ies, *sub_ie, *prof_ie, *bssid_ie, *ssid_ie;
	uint8_t sub_ies_len, prof_ie_len;


#define MBSSID_SUB_ELEM_PROF    0

	max_bssid_ind = mie[0];

	sub_ies = (uint8_t *)mie + 1;

	sub_ies_len = mie_len - 1;

	while (sub_ies_len) {
		sub_ie = (uint8_t *)get_ie((const u8 *) (sub_ies), sub_ies_len, MBSSID_SUB_ELEM_PROF);
		prof_ie = sub_ie + 2;
		prof_ie_len = sub_ie[1] + 2;

		sub_ies += prof_ie_len;
		sub_ies_len -= prof_ie_len;

		//WLAN_EID_NONTRANSMITTED_BSSID_CAPA;
		bssid_ie = (uint8_t *)get_ie((const u8 *) (prof_ie), prof_ie_len, WLAN_EID_MULTIPLE_BSSID_INDEX);

		ssid_ie = (uint8_t *)get_ie((const u8 *) (prof_ie), prof_ie_len, WLAN_EID_SSID);

		if (!bssid_ie || !ssid_ie)
			continue;

		if (wpa_nontx_bssid_get(*(bssid_ie + 2), max_bssid_ind, res->bssid, bssid) < 0)
			continue;

		os_get_reltime(&fetch_time);

		res_size = sizeof(*res) + res->ie_len + res->beacon_ie_len;

		if ((res_new = os_zalloc(res_size)) == NULL)
			continue;

		os_memcpy(res_new, res, res_size);
		os_memcpy(res_new->bssid, bssid, ETH_ALEN);

		wpa_bss_add_update(wpa_s, ssid_ie, res_new, &fetch_time);
...
#ifdef CONFIG_WM_WFA_VERSION /* add multiple bssid connection support */
	const u8 *mie;
	if ((mie = wpa_scan_get_ie(res, WLAN_EID_MULTIPLE_BSSID)) != NULL)
		wpa_mbssid_ie_parse(wpa_s, res, mie + 2, mie[1]);
#endif
...
#ifdef CONFIG_WM_WFA_VERSION
	wpa_bss_add_update(wpa_s, ssid, res, fetch_time);
#else
	bss = wpa_bss_get(wpa_s, res->bssid, ssid + 2, ssid[1]);
	if (bss == NULL)
		bss = wpa_bss_add(wpa_s, ssid + 2, ssid[1], res, fetch_time);
			...
			int new_scan)
{
	...
	wpa_printf(MSG_DEBUG, "BSS: last_scan_res_used=%u/%u",//%zu/%zu",
