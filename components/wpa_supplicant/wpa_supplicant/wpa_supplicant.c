...
static void wpa_supplicant_cleanup(struct wpa_supplicant *wpa_s)
{
	 ...
	 */
#if defined(CONFIG_GAS) || defined(CONFIG_GAS_SERVER)
	if (wpa_s->radio)
...
#ifdef CONFIG_PMKSA_CACHE_SIMPLE
    pmksa_cache_sample_update(wpa_s, ssid);
#endif
...
#ifdef CONFIG_NO_HARD_CRYPTO_EC
        timeout += 5; //soft-ec elapsed about 12s in cpu 80MHz
#endif
...
next_driver:
    ...
    wpa_s->radio = radio_add_interface(wpa_s, NULL);
#endif
...
#endif
		wpa_s->conf = wpa_config_alloc_empty(/*iface->ctrl_interface*/NULL,
