...
#ifdef CONFIG_PMKSA_CACHE_SIMPLE
#include "rsn_supp/pmksa_cache_sample.h"
#endif
...
static int compare_bss_by_rssi(const void *b1, const void *b2)
{
    struct wpa_bss *bss1 = (struct wpa_bss *)b1;
    struct wpa_bss *bss2 = (struct wpa_bss *)b2;

    if (bss1->level < bss2->level)
        return -1;
    else if (bss1->level > bss2->level)
        return 1;
    else
        return 0;
}

static int get_bss_auth_mode(struct wpa_bss *bss)
{
    u8 auth_mode; //ref wm_wifi_drv_auth_mode_t

    if (bss->caps & IEEE80211_CAP_PRIVACY)
    {
        struct wpa_ie_data adv;
        const u8 *wapi_ie = get_ie(bss->ies, bss->ie_len, WLAN_EID_BSS_AC_ACCESS_DELAY);
        const u8 *rsn_ie = get_ie(bss->ies, bss->ie_len, WLAN_EID_RSN);
        const u8 *wpa_ie = get_vendor_ie(bss->ies, bss->ie_len, WPA_IE_VENDOR_TYPE);
        if (wapi_ie && !wpa_parse_wapi_ie(wapi_ie, 2 + wapi_ie[1], &adv)) {
            auth_mode = 5;
        } else if (rsn_ie && !wpa_parse_wpa_ie(rsn_ie, 2 + rsn_ie[1], &adv)) {
            if (WPA_KEY_MGMT_SAE & adv.key_mgmt) {
                auth_mode = 4;
            } else {
                auth_mode = 3;
            }
        } else if (wpa_ie && !wpa_parse_wpa_ie(wpa_ie, 2 + wpa_ie[1], &adv) && (WPA_KEY_MGMT_PSK & adv.key_mgmt)) {
            auth_mode = 2;
        } else {
            auth_mode = 1;
        }
...
static int compare_bss_by_sec(const void *b1, const void *b2)
{
    u8 auth_mode1; //ref wm_wifi_drv_auth_mode_t
    u8 auth_mode2; //ref wm_wifi_drv_auth_mode_t

    auth_mode1 = get_bss_auth_mode((struct wpa_bss *)b1);
    auth_mode2 = get_bss_auth_mode((struct wpa_bss *)b2);

    if (auth_mode1 < auth_mode2)
        return 1;
    else if (auth_mode1 > auth_mode2)
        return -1;
    else
        return 0;
}
			  ...
			  int only_first_ssid)
{
    ...
    wpa_s->last_scan_res_used2 = wpa_s->last_scan_res_used;
#ifdef CONFIG_CONNET_ALG_OPT
    struct wpa_ssid *ssid_conf = wpa_config_get_network(wpa_s->conf, 0);
    if (ssid_conf) {
        if (!ssid_conf->sort_type) {
            /* sort by rssi */
            qsort(wpa_s->last_scan_res, wpa_s->last_scan_res_used, sizeof(struct wpa_bss *), compare_bss_by_rssi);
        } else {
            /* sort by secuty */
            qsort(wpa_s->last_scan_res, wpa_s->last_scan_res_used, sizeof(struct wpa_bss *), compare_bss_by_sec);
            for (i = 0; i < wpa_s->last_scan_res_used; i++) {
			    struct wpa_bss *bss = wpa_s->last_scan_res[i];
                if (get_bss_auth_mode(bss) < ssid_conf->auth_threshold) {
                    wpa_s->last_scan_res_used2 = i;
                    break;
                }
			}
        }
	...
	if (wpa_s->current_ssid) {
		...
		for (i = 0; i < wpa_s->last_scan_res_used2; i++) {
			struct wpa_bss *bss = wpa_s->last_scan_res[i];
...
#ifdef CONFIG_NOT_FUOUND_AP_STOP
            if (MANUAL_SCAN_REQ != wpa_s->last_scan_req) {
                if (wpa_s->no_suitable_network >= CONFIG_NOT_FUOUND_AP_NUM) {
                    wpa_s->no_suitable_network = 0;
                    wpa_s->disconnect_reason = 200;
                    wpa_supplicant_set_state(wpa_s, WPA_DISCONNECTED);
                    return 0;
                }
            } else {
...
int wpa_supplicant_fast_associate(struct wpa_supplicant *wpa_s)
{
    ...
    ssid = wpa_supplicant_pick_new_network(wpa_s);
    if (ssid) {
        if (ssid->mode == WPAS_MODE_AP && 0 != ssid->frequency) {
            wpa_dbg(wpa_s, MSG_DEBUG, "Setup a new network");
            wpa_supplicant_associate(wpa_s, NULL, ssid);
...
#ifndef CONFIG_NO_SME_AUTH_AND_ASSO
		unsigned int msec;
#endif
