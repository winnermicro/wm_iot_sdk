...
int SSLSocket_putdatas(SSL* ssl, SOCKET socket, char* buf0, size_t buf0len, PacketBuffers bufs)
{
	...
	FUNC_ENTRY;
	iovec.iov_len = (unsigned long)buf0len;
	for (i = 0; i < bufs.count; i++)
		iovec.iov_len += (unsigned long)bufs.buflens[i];
...
//	char *server_name = NULL;
	...
	if(opts->CApath || opts->trustStore){
    ...
    if( opts->CApath )
        ret = mbedtls_x509_crt_parse_path( &ssl->cacert, opts->CApath);
		if( ret != 0 ) {
	...
	{
		if( opts->trustStore ){
			ret = mbedtls_x509_crt_parse( &ssl->cacert, (const unsigned char *) opts->trustStore,
			...
			ret = 0;
			mbedtls_printf( "  CA root certificate is null.\r\n" );
		}
    }
...
#if defined(MBEDTLS_PEM_PARSE_C)
	if( opts->keyStore ){
		ret = mbedtls_x509_crt_parse( &ssl->clicert, (const unsigned char *) opts->keyStore,
		...
		ret = 0;
		mbedtls_printf( "  Client certificate is null.\r\n" );
	}
#else /* MBEDTLS_PEM_PARSE_C */
    {
        ret = 1;
	...
	{
		if( opts->keyStore && opts->privateKey ){
			ret = mbedtls_pk_parse_key( &ssl->pkey, (const unsigned char *) opts->privateKey,
			...
			mbedtls_printf( "  ok (key type: %s)\n", mbedtls_pk_get_name( &ssl->pkey ) );
		}
		else{
			ret = 0;
			mbedtls_printf( "  Client privateKey is null.\r\n" );
		}
	}
#else /* MBEDTLS_PEM_PARSE_C */
    {
	...
	//mbedtls_printf("%s: net->socket %d\n", __func__, net->socket);
	if(net->socket <= 0)
	{
		mbedtls_printf("  Connection Failed: socket %d.  Exiting\n", net->socket);
		goto exit;
	}
	FD_ZERO(&pwset);
    FD_SET(net->socket, &pwset);
    tv.tv_sec  = 0;
    tv.tv_usec = 1000;
	ret = select(net->socket+1, NULL, &pwset, NULL, &tv);
	if(ret && FD_ISSET(net->socket, &pwset))
	{
		ssl->server_fd.fd = net->socket;
	}
	else
	{
		mbedtls_printf("  Writable Failed: socket %d.  Exiting\n", net->socket);
		goto exit;
	}
    ...
    if (opts->protos && opts->protos_len)
    {
        if ((ret = mbedtls_ssl_conf_alpn_protocols(&ssl->conf, (const char **)opts->protos)) != 0)
        {
	...
	if(ssl->cert_key_init)
	{
		ret = mbedtls_ssl_conf_own_cert( &ssl->conf, &ssl->clicert, &ssl->pkey );
		if( ret != 0 )
		{
...
#if 0
	server_name = malloc(hostname_len + 1);
	if(server_name == NULL){
		goto exit;
	}
	memset(server_name, 0, sizeof(hostname_len)+1);
	memcpy(server_name, hostname, hostname_len);

	temp_p = strstr(server_name, ":");
	if(temp_p != NULL){
		*temp_p = '\0';
	}
	//printf("  server_name: %s\r\n", server_name);

	ret = mbedtls_ssl_set_hostname( &ssl->ssl, server_name );
	free(server_name);
...
int SSLSocket_continueWrite(pending_writes* pw)
{
	...
	if(pw->ssl == NULL){//if(&pw->ssl == NULL){
		return rc;
	}
	if ((rc = mbedtls_ssl_write(&pw->ssl->ssl, pw->iovecs[0].iov_base, pw->iovecs[0].iov_len)) == pw->iovecs[0].iov_len)
	{
		/* topic and payload buffers are freed elsewhere, when all references to them have been removed */
		free(pw->iovecs[0].iov_base);
		Log(TRACE_MIN, -1, "SSL continueWrite: partial write now complete for socket %d", pw->socket);
	...
	{
		int sslerror = rc; //SSLSocket_error("SSL_write", pw->ssl, pw->socket, rc, NULL, NULL);
		if (sslerror == MBEDTLS_ERR_SSL_WANT_WRITE)
			rc = 0; /* indicate we haven't finished writing the payload yet */
	}
	FUNC_EXIT_RC(rc);
	return rc;
...
int SSLSocket_getch(SSL* ssl, SOCKET socket, char* c)
{
	...
	rc = mbedtls_ssl_read( &ssl->ssl, (unsigned char *)c, (size_t)1 );
	if (rc < 0)
	{
		int err = rc;//SSLSocket_error("SSL_read - getch", ssl, socket, rc, NULL, NULL);
...
char *SSLSocket_getdata(SSL* ssl, SOCKET socket, size_t bytes, size_t* actual_len, int* rc)
{
	...
	if (*actual_len != bytes)
	{
        mbedtls_ssl_conf_read_timeout( &ssl->conf, 500);
		if ((*rc = mbedtls_ssl_read( &ssl->ssl, (unsigned char *)buf + (*actual_len), (int)(bytes - (*actual_len)))) < 0)
		{
			//*rc = SSLSocket_error("SSL_read - getdata", ssl, socket, *rc, NULL, NULL);
	...
	if (*actual_len == bytes)
	{
		...
		if so, add this socket to a new "pending SSL reads" list.
		*/
		if (mbedtls_ssl_get_bytes_avail(&ssl->ssl) > 0) /* return no of bytes pending */
			SSLSocket_addPendingRead(socket);
	}
	else /* we didn't read the whole packet */
	{
...
int SSLSocket_putdatas(SSL* ssl, SOCKET socket, char* buf0, size_t buf0len, PacketBuffers bufs)
{
	...
	iov_len = (unsigned long)buf0len;
	for (i = 0; i < bufs.count; i++)
		iov_len += (unsigned long)bufs.buflens[i];

	ptr = (char *)malloc(iov_len);
	if (!ptr)
	{
		rc = PAHO_MEMORY_ERROR;
		goto exit;
	...
	//dumpBuffer("ssl write", iov_base, iov_len );
#if 0
	while( ( rc = mbedtls_ssl_write( &ssl->ssl, (unsigned char *)iov_base, iov_len ) ) <= 0 )
    {
		printf("write error, rc: %d\r\n", rc);
        if( rc != MBEDTLS_ERR_SSL_WANT_READ && rc != MBEDTLS_ERR_SSL_WANT_WRITE )
        {
...
#else
	rc = mbedtls_ssl_write( &ssl->ssl, (unsigned char *)iov_base, iov_len );
#endif

	printf("%s: mbedtls_ssl_write rc %d, iov_len %d\n", __func__, rc, iov_len);
	if(rc != iov_len){
		rc = SOCKET_ERROR;
	}else{
		rc = TCPSOCKET_COMPLETE;
...
int SSLSocket_close(networkHandles* net)
{
	...
	if (net->ssl){
		...
		//do ret = mbedtls_ssl_close_notify( &ssl->ssl );
