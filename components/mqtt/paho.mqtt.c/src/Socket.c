        ...
        if (1)
			...
			else
			{
				...
				if (rc == EINPROGRESS || rc == EWOULDBLOCK)
				{
					...
					*pnewSd = *sock;
					if (!ListAppend(mod_s.connect_pending, pnewSd, sizeof(SOCKET)))
					{
						free(pnewSd);
						rc = PAHO_MEMORY_ERROR;
						goto exit;
...
int Socket_continueWrite(SOCKET socket)
{
	...
	for (i = 0; i < pw->count; ++i)
	{
		...
		{ /* if previously written length is in the middle of the buffer we are currently looking at,
			...
			iovecs1[++curbuf].iov_len = pw->iovecs[i].iov_len - (unsigned long)offset;
