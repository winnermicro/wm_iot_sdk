...
void Thread_start(thread_fn fn, void* parameter)
{
    ...
    CloseHandle(thread);
#else
	s_param.sched_priority = WM_TASK_MQTT_CLIENT_PRIO/*CONFIG_MQTT_WORK_TASK_PRIO*/;

	pthread_attr_init(&attr);
	pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);
	pthread_attr_setschedparam(&attr, &s_param);
	pthread_attr_setstacksize(&attr, WM_TASK_MQTT_CLIENT_STACK/*CONFIG_MQTT_WORK_TASK_STACK_SIZE*/);
	if (pthread_create(&thread, &attr, fn, parameter) != 0)
		thread = 0;
	else
	    pthread_setname_np(thread, "mqtt_client");
