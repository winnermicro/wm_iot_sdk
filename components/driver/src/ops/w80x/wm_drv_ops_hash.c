...
static int wm_w800_drv_hash_compress(wm_device_t *device, wm_drv_hash_cfg_t *ctx)
{
    wm_dt_hash_dev_t *dev = (wm_dt_hash_dev_t *)(device);
    wm_hal_hash_dev_t *hal_dev;
    int ret;
    ...
    ret     = wm_hal_hash_compress(hal_dev, ctx);
...
static int wm_w800_drv_hash_update(wm_device_t *device, wm_drv_hash_cfg_t *hash_cfg, const unsigned char *buf, uint32_t len)
{
    ...
    while (len > 0) {
        ...
        if (hash_cfg->curlen == WM_HAL_HASH_BLOCK_BYTES) {
            if (wm_w800_drv_hash_compress(device, hash_cfg) != WM_ERR_SUCCESS) {
                WM_DRV_HASH_TRACE_FAIL();
...
static int wm_w800_drv_hash_final(wm_device_t *dev, wm_drv_hash_cfg_t *hash_cfg, unsigned char *out)
{
    int32_t i;
    ...
    if (hash_cfg->curlen > 56) {
        ...
        wm_w800_drv_hash_compress(dev, hash_cfg);
        hash_cfg->curlen = 0;
    }
