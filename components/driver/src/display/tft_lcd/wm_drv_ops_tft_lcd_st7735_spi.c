 ...
 *
 * @brief DRV TFT LCD OPS Module for ST7735 with SPI Interface
 *
 */
...
int wm_st7735_set_windows(wm_device_t *dev, uint16_t x_start, uint16_t y_start, uint16_t x_end, uint16_t y_end)
{
    ...
    WM_DRV_TFT_LCD_LOG_I("wm_st7735_set_windows %d(0x%x),%d(0x%x),%d(0x%x),%d(0x%x)", x_start, x_start, y_start, y_start, x_end,
                         x_end, y_end, y_end);
    ...
    ret = lcd_send_command(dev, ST7735_CASET, LCD_CMD_TYPE_8BIT);
    ...
    ret = lcd_send_command(dev, ST7735_RASET, LCD_CMD_TYPE_8BIT);
    WM_DRV_LCD_TFT_CHECK_RETURN(ret);

    ret = lcd_send_data(dev, win, sizeof(win));
    WM_DRV_LCD_TFT_CHECK_RETURN(ret);

    ret = lcd_send_command(dev, ST7735_RAMWR, LCD_CMD_TYPE_8BIT);
    WM_DRV_LCD_TFT_CHECK_RETURN(ret);

    return ret;
}

static int wm_drv_ops_st7735_spi_init(wm_device_t *dev)
{
    int ret                    = WM_ERR_FAILED;
    ...
    ret = lcd_init_cmd(dev, st7735_init_seq, sizeof(st7735_init_seq));
    WM_DRV_LCD_TFT_CHECK_RETURN(ret);

    return ret;
}

static int wm_drv_ops_st7735_spi_draw_bitmap(wm_device_t *dev, wm_lcd_data_desc_t data_desc)
{
    int ret                   = WM_ERR_FAILED;
    wm_drv_tft_lcd_spi_t *drv = NULL;
    ...
    ret = wm_st7735_set_windows(dev, data_desc.x_start, data_desc.y_start, data_desc.x_end, data_desc.y_end);
    WM_DRV_LCD_TFT_CHECK_GOTO(ret, exit);
...
static int wm_drv_ops_st7735_spi_deinit(wm_device_t *dev)
{
...
static int wm_drv_ops_st7735_spi_set_bl(wm_device_t *dev, bool on_off)
{
    int ret                    = WM_ERR_SUCCESS;
    wm_drv_tft_lcd_spi_t *drv  = NULL;
    wm_dt_hw_tft_lcd_spi_t *hw = NULL;
...
static int wm_drv_ops_st7735_spi_set_rotation(wm_device_t *dev, wm_lcd_rotate_t degree)
{
    int ret                   = WM_ERR_SUCCESS;
    wm_drv_tft_lcd_spi_t *drv = NULL;
    ...
    switch (degree) {
        ...
        {
            rotate = ST7735_CFG_MADCTL_MY | ST7735_CFG_MADCTL_MV;
            break;
        }
        case LCD_ROTATION_180_DEGREE:
        {
            rotate = ST7735_CFG_MADCTL_MX | ST7735_CFG_MADCTL_MY;
            break;
        }
        case LCD_ROTATION_270_DEGREE:
        {
            rotate = ST7735_CFG_MADCTL_MX | ST7735_CFG_MADCTL_MV;
            break;
        }
    ...
    ret = lcd_send_command(dev, ST7735_MADCTL, LCD_CMD_TYPE_8BIT);
    WM_DRV_LCD_TFT_CHECK_RETURN(ret);
...
static int wm_drv_ops_st7735_spi_get_caps(wm_device_t *dev, wm_lcd_capabilitys_t *cap)
{
    int ret                   = WM_ERR_SUCCESS;
    wm_drv_tft_lcd_spi_t *drv = NULL;
...
static int wm_drv_ops_st7735_spi_cmd_data(wm_device_t *dev, uint16_t cmd, wm_lcd_cmd_type_t cmd_type, uint8_t *data,
                                          uint16_t data_len)
{
    int ret                   = WM_ERR_SUCCESS;
    wm_drv_tft_lcd_spi_t *drv = NULL;
...
int wm_drv_ops_st7735_add_tx_cb(wm_device_t *dev, wm_lcd_tx_callback_t callback, void *usr_data)
{
    int ret                   = WM_ERR_SUCCESS;
    wm_drv_tft_lcd_spi_t *drv = NULL;
...
int wm_drv_ops_st7735_remove_tx_cb(wm_device_t *dev)
{
    int ret                   = WM_ERR_SUCCESS;
    wm_drv_tft_lcd_spi_t *drv = NULL;
...
const wm_drv_tft_lcd_ops_t wm_drv_tft_lcd_st7735_ops = {
    .lcd_init               = wm_drv_ops_st7735_spi_init,
    .lcd_deinit             = wm_drv_ops_st7735_spi_deinit,
    .draw_bitmap            = wm_drv_ops_st7735_spi_draw_bitmap,
    .lcd_set_backlight      = wm_drv_ops_st7735_spi_set_bl,
    .lcd_set_rotation       = wm_drv_ops_st7735_spi_set_rotation,
    .lcd_get_capability     = wm_drv_ops_st7735_spi_get_caps,
    .lcd_cmd_data           = wm_drv_ops_st7735_spi_cmd_data,
    .lcd_add_tx_callback    = wm_drv_ops_st7735_add_tx_cb,
    .lcd_remove_tx_callback = wm_drv_ops_st7735_remove_tx_cb,
};