...
#define CONFIG_HARDWARE_RSA  1

#if defined(MBEDTLS_RSA_C)
#if CONFIG_HARDWARE_RSA
                       ...
                       unsigned char *output)
{
    ...
    olen = ctx->len;
#if CONFIG_HARDWARE_RSA
    if (mbedtls_mpi_bitlen(&ctx->N) <= WM_HAL_RSA_MAX_HARD_EXPTMOD_BITLEN) {
        wm_device_t* wm_drv_rsa_dev = wm_drv_rsa_init("rsa");
                                ...
                                int (*f_rng)(void *, unsigned char *, size_t), void *p_rng)
{
    ...
    if (mbedtls_mpi_bitlen(&ctx->N) <= WM_HAL_RSA_MAX_HARD_EXPTMOD_BITLEN) {
        wm_device_t* wm_drv_rsa_dev = wm_drv_rsa_init("rsa");
        MBEDTLS_MPI_CHK(wm_drv_rsa_expt_mod(wm_drv_rsa_dev, &ctx->Vi, &ctx->Vi, &ctx->E, &ctx->N));
    } else {
                        ...
                        unsigned char *output)
{
    ...
    if (mbedtls_mpi_bitlen(&ctx->N) <= WM_HAL_RSA_MAX_HARD_EXPTMOD_BITLEN) {
        wm_device_t* wm_drv_rsa_dev = wm_drv_rsa_init("rsa");
        MBEDTLS_MPI_CHK(wm_drv_rsa_expt_mod(wm_drv_rsa_dev, &T, &T, D, &ctx->N));
    } else {
     ...
     */
#if CONFIG_HARDWARE_RSA
    if (mbedtls_mpi_bitlen(&ctx->P) <= WM_HAL_RSA_MAX_HARD_EXPTMOD_BITLEN && mbedtls_mpi_bitlen(&ctx->Q) <= WM_HAL_RSA_MAX_HARD_EXPTMOD_BITLEN) {
        wm_device_t* wm_drv_rsa_dev = wm_drv_rsa_init("rsa");
        MBEDTLS_MPI_CHK(wm_drv_rsa_expt_mod(wm_drv_rsa_dev, &TP, &T, DP, &ctx->P));
        MBEDTLS_MPI_CHK(wm_drv_rsa_expt_mod(wm_drv_rsa_dev, &TQ, &T, DQ, &ctx->Q));
    } else {
    ...
    if (mbedtls_mpi_bitlen(&ctx->N) <= WM_HAL_RSA_MAX_HARD_EXPTMOD_BITLEN) {
        wm_device_t* wm_drv_rsa_dev = wm_drv_rsa_init("rsa");
        MBEDTLS_MPI_CHK(wm_drv_rsa_expt_mod(wm_drv_rsa_dev, &C, &T, &ctx->E, &ctx->N));
    } else {
